...

{
	...
	_query = [format["SELECT price FROM ressources WHERE name='%1'", _ressourceName], 2] call EXTDB2_fnc_async;
	if (_query isEqualTo []) then
	{
		diag_log format["[ERROR] Market ressource %1 was not saved into DB. Fixed", _ressourceName];
		[format["INSERT INTO ressources (name,price) VALUES ('%1','%2')", _ressourceName, getNumber(_x >> "market" >> "max")], 1] call EXTDB2_fnc_async;
