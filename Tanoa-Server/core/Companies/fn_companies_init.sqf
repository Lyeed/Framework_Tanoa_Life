...

{
	...
	if ((_x select 9) isEqualTo 0) then
	{
		...
		[
			...
			[format["SELECT TRANS_from,TRANS_date,TRANS_type,TRANS_value,TRANS_reason FROM companies_bank_transactions WHERE COMPANY_plate='%1'", (_x select 4)], 2] call EXTDB2_fnc_async,
			(_x select 15)
