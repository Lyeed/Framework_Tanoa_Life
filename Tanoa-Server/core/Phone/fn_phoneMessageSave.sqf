...

{
	...
	{
		_queryResult = [format["SELECT PHONE_messages FROM players WHERE PHONE_number='%1'", _x], 2] call EXTDB2_fnc_async;
		if (!(_queryResult isEqualTo [])) then
		{
			...
			[format["UPDATE players SET PHONE_messages='%1' WHERE PHONE_number='%2'", [_old_messages] call AlysiaServer_fnc_mresArray, _x], 1] call EXTDB2_fnc_async;
		};
	};
