...
{
	if (["G_Bandanna_blk", false] call AlysiaClient_fnc_handleItem) then
	{
		g_action_inUse = true;
		player playMove "AinvPercMstpSnonWnonDnon_Putdown_AmovPercMstpSnonWnonDnon";
		waitUntil {((animationState player) isEqualTo "ainvpercmstpsnonwnondnon_putdown_amovpercmstpsnonwnondnon")};
		[] remoteExec ["AlysiaClient_fnc_silence_put_target", _target];
		g_action_inUse = false;
	} else {
		["Vous n'avez pas de baillon."] call AlysiaClient_fnc_info;
	};
} else {
